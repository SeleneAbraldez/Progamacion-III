...
// Recibe el ID del auto a ser borrado más el JWT. Si el perfil es ‘ propietario ’ se borrará de
// la base de datos. Caso contrario, se mostrará el mensaje correspondiente (indicando que usuario
// intentó realizar la acción).
// Retorna un JSON (éxito: true/false; mensaje: string; status: 200/418)
$app->delete('[/]',\Auto::class . ':EliminarAutApi')->add(\MW::class . '::VerificarPropietario')->add(\MW::class . ':VerificarToken');

// (PUT) Modificar los autos por ID.
// Recibe el JSON del auto a ser modificado más el JWT. Si el perfil es ‘ propietario ’ o
// ‘ encargado ’ se modificará de la base de datos. Caso contrario, se mostrará el mensaje
// correspondiente (indicando que usuario intentó realizar la acción).
// Retorna un JSON (éxito: true/false; mensaje: string; status: 200/418)
$app->put('[/]',\Auto::class . ':ModificarAutApi')->add(\MW::class . ':VerificarEncargado')->add(\MW::class . ':VerificarToken');
